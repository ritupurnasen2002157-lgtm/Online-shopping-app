...
function forgotPassword(email) {
    ...
    return { success: true, message: `Password reset link sent to ${email}. Use "demo123" to login.` };
}
...
function updateFilterUI() {
    document.querySelectorAll('.category-card').forEach(card => {
        const cardCategory = card.dataset.category || card.querySelector('h3')?.textContent.trim();
...
function renderCart() {
    ...
    container.innerHTML = cart.map(item => {
        const needsSizeSelector = !['Beauty Products', 'Essentials'].includes(item.category);
...
function showAccountPage(section = 'account') {
    ...
    document.body.insertBefore(accountPage, document.querySelector('footer'));
...
function setupEventListeners() {
    ...
    document.querySelectorAll('.tab-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (tab === 'login') document.getElementById('loginFormContainer').classList.add('active');
            else document.getElementById('registerFormContainer').classList.add('active');
